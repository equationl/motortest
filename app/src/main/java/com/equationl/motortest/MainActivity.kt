...
import kotlinx.android.synthetic.main.dialog_main_help.view.*
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreateOptionsMenu(menu: Menu): Boolean {
        ...
        val item = menu.findItem(R.id.main_action_high_accuracy)
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        ...
        when (item.itemId) {
            R.id.main_action_high_accuracy -> {
                if (item.isChecked) {
            ...
            R.id.main_action_run_background -> {
                if (item.isChecked) {
    ...
    private fun checkDevice() {
        ...
        if (!hasVibrator) {
            Toast.makeText(applicationContext, R.string.main_toast_notSupportVibrator, Toast.LENGTH_LONG).show()
            finish()
        }

        if (!vibrator.hasAmplitudeControl()) {
            hasAmplitudeControl = false
            main_text_amplitude.text = getString(R.string.main_text_notSupport_amplitude)
    ...
    private fun initPreVibrator() {
        if (Build.VERSION.SDK_INT < 29) {
            main_text_predefined.text = getString(R.string.main_text_predefined_notSupport)
    ...
    private fun listenerBtn() {
        ...
        main_btn_pre_start.setOnClickListener {
            when (main_spinner_pre.selectedItemPosition) {
                ...
                1 -> createOffBeat()
                2 -> createRock()
                3 -> Toast.makeText(applicationContext, getString(R.string.main_btn_pre_wait), Toast.LENGTH_LONG).show()
            }
        }
        ...
        main_edit_diy_timings.setOnFocusChangeListener { _, hasFocus ->
            if (hasFocus) {
                ...
                if (main_edit_diy_timings.text.toString() == "") {
                    main_layout_diy_timings.error = getString(R.string.main_edit_diy_tip_text_empty)
                }
            }
        }
        ...
        main_edit_diy_amplitudes.setOnFocusChangeListener { _, hasFocus ->
            if (hasFocus) {
                ...
                if (main_edit_diy_amplitudes.text.toString() == "") {
                    main_layout_diy_amplitudes.error = getString(R.string.main_edit_diy_tip_text_empty)
                }
            }
        }
        ...
        main_edit_diy_repeat.setOnFocusChangeListener { _, hasFocus ->
            if (hasFocus) {
                ...
                if (main_edit_diy_repeat.text.toString() == "") {
                    main_layout_diy_repeat.error = getString(R.string.main_edit_diy_tip_text_empty)
                }
            }
        }
        ...
        main_btn_diy_menu.setOnClickListener { view ->
            ...
            inflater.inflate(R.menu.menu_main_btn_diy , popup.menu)
        ...
        main_fab_help.setOnClickListener {
            ...
            val layout = View.inflate(this, R.layout.dialog_main_help, null)
            MaterialAlertDialogBuilder(this)
                .setTitle(getString(R.string.main_fab_help_title))
                .setPositiveButton(getString(R.string.main_fab_help_btn_close), null)
                .setView(layout)
                .show()
            layout.main_dialog_text_content.movementMethod = LinkMovementMethod.getInstance()
            layout.main_dialog_text_content.text = Utils.text2html(getString(R.string.main_dialog_help_content_summary))
            layout.main_dialog_tab_layout.addOnTabSelectedListener(object :
                ...
                override fun onTabSelected(tab: TabLayout.Tab) {
                    when (tab.position) {
                        ...
                        1 -> {
                            layout.main_dialog_text_content.text = Utils.text2html(getString(R.string.main_dialog_help_content_usage))
                        }
                        2 -> {
                            layout.main_dialog_text_content.text = Utils.text2html(getString(R.string.main_dialog_help_content_about))
                        }
                        3 -> {
                            layout.main_dialog_text_content.text = Utils.text2html(getString(R.string.main_dialog_help_content_update))
                        }
                    }
                }
    ...
    private fun diyClickOpen() {
        ...
        alertDialog = MaterialAlertDialogBuilder(this)
            .setTitle(getString(R.string.main_diy_open_choise_title))
            .setPositiveButton(getString(R.string.main_diy_open_choise_btn_close), null)
    ...
    private fun checkDiyData(): List<String>? {
        ...
        try {
            ...
            main_layout_diy_repeat.error = getString(R.string.main_edit_diy_tip_number_error)
            return null
        }
        ...
        for (dataT in timingsT) {
            ...
            try {
                ...
                main_layout_diy_timings.error = getString(R.string.main_edit_diy_tip_number_error)
                return null
            }
            if (data < 0) {
                main_layout_diy_timings.error = getString(R.string.main_diy_check_timings_less_zero)
                return null
            }
        }
        ...
        for (dataT in amplitudeT) {
            ...
            try {
                ...
                main_layout_diy_amplitudes.error = getString(R.string.main_edit_diy_tip_number_error)
                return null
            }
            if (data < 0 || data > 255) {
                main_layout_diy_amplitudes.error = getString(R.string.main_diy_check_amplitude_number_error)
                return null
            }
        }
        ...
        if (amplitudeT.size != timingsT.size) {
            return if (amplitudeT.size > timingsT.size) {
                main_layout_diy_amplitudes.error = getString(R.string.main_diy_check_amp_bigThan_tim)
                null
            } else {
                main_layout_diy_timings.error = getString(R.string.main_diy_check_tim_bigThan_amp)
                null
            }
        }

        if (repeateI >= amplitudeT.size) {
            main_layout_diy_repeat.error = getString(R.string.main_diy_check_reapeat_outOfIndex)
            return null
        }
        if (repeateI < -1) {
            main_layout_diy_repeat.error = getString(R.string.main_diy_check_repeat_wrong_index)
            return null
        }
    ...
    private fun diyClickSave() {
        ...
        MaterialAlertDialogBuilder(this)
            .setTitle(getString(R.string.main_diy_save_fileName_title))
            .setView(editText)
            .setPositiveButton(getString(R.string.main_diy_save_fileName_btn_sure)) { _, _ ->
                val text = editText.text.toString()
                if (text == "") {
                    Toast.makeText(applicationContext, getString(R.string.main_diy_save_fileName_isEmpty),Toast.LENGTH_LONG).show()
                }
                else {
                    val databaseHelper = DatabaseHelper.getInstance(applicationContext)
                    databaseHelper.insert(VibrationEffects(null, timings, amplitude, Integer.parseInt(repeateI), text, System.currentTimeMillis()))
                    Toast.makeText(applicationContext, getString(R.string.main_diy_save_success),Toast.LENGTH_LONG).show()
                }
            }
            .show()
    }
    ...
    private fun diyClickImport() {
        val editText = EditText(applicationContext)
        MaterialAlertDialogBuilder(this)
            .setTitle(getString(R.string.main_diy_import_dialog_title))
            .setView(editText)
            .setPositiveButton(getString(R.string.main_diy_import_dialog_btn_sure)) { _, _ ->
                var text = editText.text.toString()
                if (text == "") {
                    Toast.makeText(applicationContext, getString(R.string.main_diy_import_text_isEmpty),Toast.LENGTH_LONG).show()
                }
                else {
                    try {
                        ...
                        Toast.makeText(applicationContext, getString(R.string.main_diy_import_fail),Toast.LENGTH_LONG).show()
                    }
                }
            }
            .show()
